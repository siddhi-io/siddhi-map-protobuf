...
import io.siddhi.extension.map.protobuf.autogenerated.MessageWithUnderscore;
...
public class TestCaseOfProtobufSourceMapper {
    ...
    @Test
    public void protobufSourceMapperTestNestedMessageTypes() throws Exception {
            ...
            public void receive(Event[] events) {
                ...
                for (Event event : events) {
                    switch (count.incrementAndGet()) {
                        case 1: {
                            AssertJUnit.assertEquals("Test 01", event.getData(0));
                            AssertJUnit.assertEquals(1000, event.getData(1));
                            AssertJUnit.assertEquals(innerObj, event.getData(2));
                            AssertJUnit.assertEquals(innerList, event.getData(3));
                            break;
                        }
                        default:
                            AssertJUnit.fail();
        ...
        siddhiAppRuntime.start();
        InMemoryBroker.publish("test01", nestedMessage.toByteArray());
        SiddhiTestHelper.waitForEvents(waitTime, 1, count, timeout);
        //assert event count
        AssertJUnit.assertEquals("Number of events", 1, count.get());
        siddhiAppRuntime.shutdown();
    ...
    @Test
    public void protobufSourceMapperTestWithUnderscore() throws Exception {
        String streams = "" +
                "@App:name('TestSiddhiApp')" +
                "@source(type='inMemory', topic='test01'," +
                " @map(type='protobuf', class='" + packageName + ".MessageWithUnderscore')) " +
                "define stream FooStream (string_value string, int_value int, doubleValue_ double); " +
                "define stream BarStream (string_value string, int_value int, doubleValue_ double); ";
        String query = "" +
                "from FooStream " +
                "select * " +
                "insert into BarStream; ";
                ...
                for (Event event : events) {
                    switch (count.incrementAndGet()) {
                        case 1:
                            AssertJUnit.assertEquals("Test 01", event.getData(0));
                            AssertJUnit.assertEquals(1000, event.getData(1));
                            AssertJUnit.assertEquals(100.123, event.getData(2));
                            break;
                        case 2:
                            AssertJUnit.assertEquals("Test 02", event.getData(0));
                            AssertJUnit.assertEquals(2000, event.getData(1));
                            AssertJUnit.assertEquals(200.1234, event.getData(2));
                            break;
                        default:
                            AssertJUnit.fail();
                    }
        ...
        siddhiAppRuntime.start();
        MessageWithUnderscore messageWithUnderscore1 = MessageWithUnderscore.newBuilder()
                .setIntValue(1000)
                .setStringValue("Test 01")
                .setDoubleValue(100.123)
                .build();
        MessageWithUnderscore messageWithUnderscore2 = MessageWithUnderscore.newBuilder()
                .setIntValue(2000)
                .setStringValue("Test 02")
                .setDoubleValue(200.1234)
                .build();
        InMemoryBroker.publish("test01", messageWithUnderscore1.toByteArray());
        InMemoryBroker.publish("test01", messageWithUnderscore2.toByteArray());
        SiddhiTestHelper.waitForEvents(waitTime, 2, count, timeout);
        //assert event count
        AssertJUnit.assertEquals("Number of events", 2, count.get());
        siddhiAppRuntime.shutdown();
